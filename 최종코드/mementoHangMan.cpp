...
public:
   Memento(nState state) :state(state) {}
   Memento() {}
   friend class NState2;
   void memento(nState nowState, string strInput)
   {
      NState2 test{ nowState };
      test.save(nowState);
      if (strInput == ".") { test.undo(); }
   }
};
class NState2
{
   ...
   int current;
public:
   explicit NState2(const nState state) :state(state)
   {
      changes.emplace_back(make_shared<Memento>(state));
      current = 0;
   }

   shared_ptr<Memento>save(nState saveState)
   {
      state = saveState;
      auto m = make_shared<Memento>(state);
      changes.push_back(m);
      ++current;
      return m;
   }
...
// 제작-------------------------------------------------- 인데 합치고 보니 필요한지 모르겠어서 생각중
      nState nowState = {
         nowState.life = life, 
         nowState.score = score, 
         nowState.strQuestion = strQuestion,
         nowState.pastWord = pastWord 
      };
//-------------------------------------------------------
